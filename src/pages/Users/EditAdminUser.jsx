...
function EditAdminUser(props) {
  ...
  ******************/
  const getAllUsers = async (id) => {
    ...
    if (response.status === 1) {
      let { firstname, lastname, emailId, staticRole, status } = response.data;
      setFirstName(firstname);
      setLastName(lastname);
      setEmail(emailId);
      if(staticRole) setRole({ value: staticRole._id, label: staticRole.role });
      setStatus(status);
    }
  };
  /******************* 
  ...
  ******************/
  ...
  /******************* 
  @Purpose : Used for update admin profile
  @Parameter : {}
  @Author : INIC
  ******************/
  const editAdminProfile = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (response.status === 1) {
          showMessageNotification("User updated successfully", "success");
                          ...
                          <div className="form-group d-flex align-items-center mb-md-4 mb-3">
                            ...
                            <div className="w-100">
                              <input
                                ...
                                name="firstName"
                                placeholder={lang(
                                  "Usermanagement.edituser.labels.firsname"
                                )}
                                value={firstName}
                                onChange={(e) => {
                          ...
                          <div className="form-group d-flex align-items-center mb-md-4 mb-3">
                            ...
                            <div className="w-100">
                              <input
                                ...
                                name="email"
                                placeholder={lang(
                                  "Usermanagement.edituser.labels.email"
                                )}
                                type="email"
                                value={email}
                                onChange={(e) => {
...
@Author : INIC
******************/
export default connect(mapStateToProps, { callApi })(EditAdminUser);
