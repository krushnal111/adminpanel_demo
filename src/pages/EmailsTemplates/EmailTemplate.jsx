...
import SunEditorAdd from "../../components/Editor/SunEditorAdd";
import { showMessageNotification } from "./../../utils/Functions"; //Utility functions
import { CopyToClipboard } from "react-copy-to-clipboard";
import { ADMIN_URL } from "../../config";
var { Option } = Select;
/******************* 
@Purpose : Used for email tamplet view
@Parameter : props
@Author : INIC
******************/
function EmailTemplate(props) {
  ...
  const [, setUserFirstName] = useState("");
  const [, setUserLastName] = useState("");
  ...
  useEffect(() => {
    ...
    if (props.adminData) {
      let { firstname, lastname, emailId: email } = props.adminData;
  ...
  /******************* 
  @Purpose : Used for get email template details
  @Parameter : id
  @Author : INIC
  ******************/
  const getEmailTemlateDetails = async (id) => {
    ...
    if (response.status === 1) {
      let { emailKey, subject: sub, fromEmailId, emailContent } = response.data;

      setTemplateTitle(emailKey);
      setSubject(sub);
  ...
  /******************* 
  @Purpose : Used for add email template
  @Parameter : event
  @Author : INIC
  ******************/
  ...
  ******************/
  ...
  /******************* 
  @Purpose : Used for update tags
  @Parameter : {}
  @Author : INIC
  ******************/
  const setUpddateTags = (value) => {
    setTags(value);
    if (emailTemplates === "") {
      setEmailTemplates(`{{{${value}}}}`);
    } else {
      var str1 = `{{{${value}}}}`;
      var str2 = emailTemplates;
  ...
  ******************/
  ...
  /******************* 
  @Purpose : Used for cancle action
  @Parameter : {}
  @Author : INIC
  ******************/
  const onCancel = () => {
                  ...
                  <div className="col-md-6">
                    <div className="form-group mb-3 mb-md-5">
                      ...
                      <Select
                        ...
                        showSearch
                        onChange={(value) => setUpddateTags(value)}
                        value={tags}
                        onFocus={GetAllTags}
                      >
                        {array.map((tag, key) => {
...
@Author : INIC
******************/
export default connect(mapStateToProps, { callApi })(withRouter(EmailTemplate));
