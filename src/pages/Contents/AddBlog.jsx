...
import { ADMIN_URL, IMG_URL } from "../../config"; // Default configuration
import { useSorting } from "../../hooks";
import Datetime from "react-datetime"; // Date time function
import "react-datetime/css/react-datetime.css";
import BlogEditorAdd from "../../components/Editor/BlogEditorAdd";
import { showMessageNotification } from "./../../utils/Functions"; //Utility functions
/******************* 
@Purpose : Used for add blog data
@Parameter : props
@Author : INIC
******************/
function AddBlog(props) {
  ...
  ******************/
  const validateForm = () => {
    ...
    if (!pageTitle.trim()) formError.pageTitle = "*Page title is required";
    else if (pageTitle.length > 70)
      formError.pageTitle = "*Page title can't be more than 70 characters";
    else formError.pageTitle = "";

    if (!selectedFile) formError.selectedFile = "*Image is required";
    else formError.selectedFile = "";

    if (!blogCategory) formError.blogCategory = "*Blog category is required";
    else formError.blogCategory = "";

    if (!blogTitle.trim()) formError.blogTitle = "*Blog title is required";
    else if (blogTitle.length > 70)
      formError.blogTitle = "*Blog title can't be more than 70 characters";
    else formError.blogTitle = "";

    if (!blogSlug.trim()) formError.blogSlug = "*Slug is required";
    else formError.blogSlug = "";

    if (!metaTitle.trim()) formError.metaTitle = "*Meta title is required";
    else formError.metaTitle = "";

    if (!metaKeyword.trim())
      formError.metaKeyword = "*Meta keyword is required";
    else formError.metaKeyword = "";

    if (!metaDescription.trim())
      formError.metaDescription = "*Meta description is required";
    else formError.metaDescription = "";

    if (!caption.trim()) formError.caption = "*Content is required";
    else if (caption.length < 50)
      formError.caption = "*Caption must contain minimum 50 characters";
    else formError.caption = "";

    if (
      formError.selectedFile !== "" ||
      formError.blogCategory !== "" ||
      formError.blogTitle !== "" ||
      formError.blogSlug !== "" ||
      formError.metaKeyword !== "" ||
      formError.metaDescription !== "" ||
      formError.caption !== "" ||
      formError.metaTitle !== ""
    )
      isFormValid = false;
  ...
  /******************* 
  @Purpose : Used for fetch blog categories 
  @Parameter : {}
  @Author : INIC
  ******************/
  ...
  /******************* 
  @Purpose : Used for add CMS pages
  @Parameter : {e}
  @Author : INIC
  ******************/
  const addCmsPage = async (e) => {
    ...
    if (validateForm()) {
      ...
      if (response.status === 1) {
        showMessageNotification("Blog added successfully", "success", 1200);
        props.history.push("/blogListing");
      }
    }
  };
  /******************* 
  @Purpose : Used for generator slug
  @Parameter : {e}
  @Author : INIC
  ******************/
  const handleSlugGenerator = async (e) => {
    ...
    let body = {
      ...
      if (response.status === 1) {
        ...
        showMessageNotification(response.message, "error", 1200);
      }
  };
  /******************* 
  ...
  ******************/
  ...
  const options = [];
  blogCategoryListing.forEach((item) => {
    options.push({
            ...
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb">
                ...
                <li className="breadcrumb-item active" aria-current="page">
                  {lang("ContentManagement.CMS.add")}
                </li>
              </ol>
            </nav>
